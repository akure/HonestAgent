...
The formerly pinned development dependency **pytest 8.3.4** was affected by **CVE-2025-71176 / GHSA-6w46-j5rx-g56g**. The advisory covers pytest versions below **9.0.3** and describes insecure `/tmp/pytest-of-{user}` handling that can permit a local UNIX user to cause denial of service or potentially gain privileges. The issue is relevant to shared CI or multi-user build hosts, but pytest is not imported by the production runtime. The repository now pins pytest 9.0.3.

The remediation was applied in commit `4ad8c3e`: both dependency declarations now require pytest 9.0.3 or newer within the tested major range. A clean environment passed all 80 tests.

The scan also reported multiple Starlette advisories against the formerly resolved transitive version `0.41.3`. The safe runtime remediation was a coordinated FastAPI upgrade: the repository now uses FastAPI 0.137.1, resolving Starlette 1.6.0. The clean upgrade passed all 80 tests and `pip-audit`; malformed-request and production-like load evidence remains a conditional-pilot requirement. Starlette should not be force-installed independently.
