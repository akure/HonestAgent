...
- STD-10C immutable audit and evidence retention: lock-serialized append, fsync-before-return durability, integrity-checked scoped retrieval, non-destructive retention filtering, strict required fields, and fail-closed malformed-record handling. External WORM, replication, legal-hold, and production storage evidence remain out of scope. Evidence: `docs/development/sprint_traces/sprint_std10c_audit_retention_20260903.md`.
- STD-10D operational controls: durable actor-attributed kill-switch events, read-only execution snapshots, deterministic threshold alerts, and dashboard payloads. External paging, production telemetry, operator authentication, and break-glass evidence remain out of scope. Evidence: `docs/development/sprint_traces/sprint_std10d_operations_20260903.md`.
- STD-10E deployment packaging and commercial boundaries: fail-closed managed-environment manifest validation for digest, SBOM, audit, operator auth, TLS, secrets, and side-effect mode, plus explicit protocol/kernel/enterprise-service separation. Artifact builds, billing, hosted services, licensing enforcement, and production evidence remain out of scope. Evidence: `docs/development/sprint_traces/sprint_std10e_deployment_packaging_20260903.md`.
- STD-10F release packaging verification: deterministic source-manifest hashing, required-input and Dockerfile safety checks, packaging-tool inventory, honest `NOT_RUN` results for unavailable build/SBOM/scan tools, and a reproducible offline verifier command. No image, SBOM, vulnerability, signing, or production claims were made. Evidence: `docs/development/sprint_traces/sprint_std10f_release_packaging_20260903.md`.
- STD-10G release verification gate: fail-closed prerequisite inventory for image builder, SBOM generator, signer, vulnerability scanner, and deployment client. The local environment lacks all required tools, so real build, SBOM, signing, scanning, and rehearsal remain `NOT RUN`; no release claim is made. Evidence: `docs/development/sprint_traces/sprint_std10g_release_gate_20260904.md`.
